...
def Analysis():
    ...
    count_obj = Mongo.coll['Plugin'].group([], {}, {'count': 0},'function(doc,prev){prev.count = prev.count + doc.count}')
    # vul = int(Mongo.coll['Plugin'].group([], {}, {'count': 0},'function(doc,prev){prev.count = prev.count + doc.count}')[0]['count'])
    if len(count_obj) > 0:
        vul = int(count_obj[0]['count'])
    else:
        vul = 0
...
def Summary():
    page = int(request.args.get('page', '1'))
    if len(request.args):
        params_key = next(iter(request.args))
        params_value = request.args.get(params_key, None)
        a = params_key + params_value
        # cursor = Mongo.coll['Result'].find().limit(page_size).skip((page - 1) * page_size)
        if params_key == "title":
            tasks = Mongo.coll['Task'].find({"title": params_value})
        else:
            tasks = Mongo.coll['Task'].find()
        t_list = []
        for t in tasks:
            ...
            if len(lastscan) == 0:
                ...
                cursor = Mongo.coll['Result'].find({'task_id': t['_id'], 'task_date': taskdate})
            for _ in cursor:  # 单任务详情
                if str(_.get(params_key, None)) == params_value or _['vul_info'].get('vul_level', None) == params_value:
                    hostname = Mongo.coll['Info'].find_one({'ip': _['ip']})
                    if hostname:
                        _['hostname'] = hostname['hostname']
                    else:
                    ...
                    _['lastscan'] = taskdate
                    t_list.append(_)
        return render_template('summary.html', cursor=t_list)
    else:
        # cursor = Mongo.coll['Result'].find().limit(page_size).skip((page - 1) * page_size)
        tasks = Mongo.coll['Task'].find({})
        t_list = []
